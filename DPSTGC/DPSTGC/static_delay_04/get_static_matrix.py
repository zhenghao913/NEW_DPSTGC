...
def data_transformation(graph_signal_matrix_filename, num_of_hours, num_of_days, num_of_weeks):
    ...
    #train_x torch.Size([10181, 307, 12])
    ...
    #print("train_x", train_x.shape) # train_x train_x torch.Size([10181, 307, 12])
    train_x = train_x[:, :, 0:1, :]  # [开始位置：终止位置]前闭后开，只剩下一个总流量
    train_target = file_data['train_target']
    train_x = np.squeeze(train_x, axis=2)
    train_x = torch.from_numpy(train_x)
    print("train_x", train_x.shape) #得到train_x torch.Size([10181, 307, 12])
    #舍弃最后不够一小时的点，即10181 - 5 = 10176 = 848 * 12
    train_x = train_x[5:, :, :].reshape(848, 12, 307, 12)[:, 0, :, :]
    print("train_x.shape", train_x.shape)

    train_x = torch.squeeze(train_x)#(848, 12, 307, 12)
    train_x = train_x.permute(1, 0, 2).reshape(307, -1)
