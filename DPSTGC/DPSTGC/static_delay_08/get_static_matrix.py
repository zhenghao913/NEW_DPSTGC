...
def data_transformation(graph_signal_matrix_filename, num_of_hours, num_of_days, num_of_weeks):
    ...
    train_x = file_data['train_x']  # (10181, 307, 3, 12)#3种交通测量量，总流量、平均速度和平均占用率
    train_x = train_x[:, :, 0:1, :]  # [开始位置：终止位置]前闭后开，只剩下一个总流量
    train_target = file_data['train_target']  # (10181, 307, 12)    train timestamp: (10181, 1),per_hour_12value
    train_x = np.squeeze(train_x, axis=2)
    train_x = torch.from_numpy(train_x)#得到tensor(10181, 307, 12)
    #舍弃最后不够一小时的点，即10699 - 7 = 10692 = 891 * 12
    #得到节点的实际时间序列 #torch.Size([10699, 170, 12])
    train_x = train_x[:-7, :, :].reshape(891, 12, 170, 12)[:, 0, :, :]
    print(train_x.shape)

    #在每个小时内进行聚合（848, 1, 307, 12）
    train_x = torch.squeeze(train_x)#（848, 307, 12）
    train_x = train_x.permute(1, 0, 2).reshape(170, -1)
